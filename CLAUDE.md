...
PRレビュー時は以下の観点を必ず確認すること。

### 環境変数
- 環境変数を追加・変更・削除する場合は、必ず `.env.example` も同様に更新する

### セキュリティ
- Supabase のすべてのテーブルに RLS（Row Level Security）が適切に設定されているか
- `createSupabaseClient(request, responseHeaders)` を正しく使用しているか（`responseHeaders` をレスポンスに含めているか）
- ユーザー入力のサニタイズや検証が適切に行われているか
...
### モバイル対応
- コンテナの高さに `h-dvh` を使っているか（`h-screen` は使わない）
- セーフエリアに `env(safe-area-inset-*)` で対応しているか

### 写真処理
- 写真アップロード前にクライアント側で WebP 変換・最大1920px へのリサイズを行っているか
- 写真の表示に署名付きURL（`createSignedUrl`、有効期限1時間）を使用しているか

### DB マイグレーション
- DBスキーマを変更する場合は `supabase/migrations/` にマイグレーションファイルを作成しているか
